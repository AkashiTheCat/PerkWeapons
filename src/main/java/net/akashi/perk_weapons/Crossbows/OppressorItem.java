...
public class OppressorItem extends BaseCrossbowItem {
	...
	public void onInventoryTick(ItemStack stack, Level level, Player player, int slotIndex, int selectedIndex) {
		...
		Vec3 max = eyePosition.add(viewVec.scale(AFFECT_RANGE));
		AABB searchBox = player.getBoundingBox().expandTowards(
				viewVec.scale(AFFECT_RANGE)).inflate(1.0D, 1.0D, 1.0D);

		EntityHitResult result = ProjectileUtil.getEntityHitResult(player, eyePosition, max, searchBox,
				isVisible, AFFECT_RANGE * AFFECT_RANGE);
		if (result == null) {
			return;
		}

		if (result.getEntity() instanceof LivingEntity target && player.hasLineOfSight(target)) {
